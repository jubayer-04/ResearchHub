           ...
           // $userAnnotations = $conn->query("SELECT COUNT(*) FROM annotations WHERE user_id = " . $_SESSION['user_id'])->fetch_row()[0];
            ...
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap: 1.5rem; margin-top: 3rem;">
                <div class="card">
                    ...
                    <a href="index.php" class="btn btn-primary btn-block" style="margin-top: 1rem;">Go to Papers</a>
                </div>
                <div class="card">
                    <h3>📝 Add New Paper</h3>
                    <p>Upload and create a new research paper.</p>
                    <a href="paper_form.php" class="btn btn-primary btn-block" style="margin-top: 1rem;">Create Paper</a>
                </div>
                <?php if ($_SESSION['role'] === 'admin'): ?>
                <div class="card">
                    <h3>👥 Manage Users</h3>
